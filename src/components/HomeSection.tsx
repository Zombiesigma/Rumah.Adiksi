  ...
  return (
    <div className="space-y-6 py-2">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-5">
        <div>
          <span className="text-xs uppercase font-mono tracking-widest text-brand-gold font-bold flex items-center gap-1.5 mb-1.5"><Compass className="w-4 h-4 animate-spin-slow" /> Inkubator Talenta Pesisir</span>
          <h2 className="text-3xl font-extrabold tracking-tight text-white font-serif md:text-4xl">Rumah Kreativitas: <span className="text-brand-gold italic">Dari Candu jadi Karya</span></h2>
        </div>
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          <button 
            onClick={() => setActiveTab('manifesto')} 
            className="px-4 py-2.5 bg-gradient-to-r from-amber-500/10 to-brand-gold/10 hover:from-amber-500/20 text-brand-gold border border-brand-gold/30 hover:border-brand-gold text-xs font-bold rounded-xl flex items-center gap-2 transition-all active:scale-95 cursor-pointer"
          >
          ...
          <button 
            ...
            className="px-4 py-2.5 bg-neutral-800 hover:bg-neutral-700 text-white border border-white/5 hover:border-white/10 text-xs font-bold rounded-xl flex items-center gap-2 transition-all active:scale-95 cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
        ...
        <div className="md:col-span-12 lg:col-span-8 bg-brand-card rounded-3xl overflow-hidden relative border border-white/5 min-h-[460px] flex flex-col justify-between shadow-2xl group hover:border-brand-gold/25 transition-all duration-300">
          ...
          <div className="relative z-10 p-6 md:p-8 space-y-4 max-w-xl">
            <div className="space-y-2">
              ...
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white font-serif leading-tight tracking-tight">{slides[activeSlide]?.title}</h1>
            ...
            <div className="flex flex-wrap gap-2.5 pt-2">
              <button 
                onClick={() => setActiveTab('gallery')} 
                className="px-5 py-2.5 bg-brand-gold hover:bg-amber-500 text-brand-charcoal text-[11px] font-mono font-black uppercase rounded-full tracking-wider shadow-lg shadow-brand-gold/15 cursor-pointer active:scale-95 transition-all"
              >
                Lihat Galeri
              </button>
              ...
              <button 
                ...
                className="px-5 py-2.5 bg-white/10 hover:bg-white/15 text-white border border-white/10 hover:border-white/20 text-[11px] font-mono font-black uppercase rounded-full tracking-wider cursor-pointer active:scale-95 transition-all"
              >
                Reservasi Tiket
              </button>
              ...
              <button 
                ...
                className="px-5 py-2.5 bg-neutral-850/90 hover:bg-neutral-800 text-brand-gold border border-brand-gold/20 hover:border-brand-gold/40 text-[11px] font-mono font-black uppercase rounded-full tracking-wider flex items-center gap-1.5 shadow-lg cursor-pointer active:scale-95 transition-all"
              >
        ...
        <div className="md:col-span-12 lg:col-span-4 bg-gradient-to-br from-brand-subcard to-brand-card rounded-3xl p-6 border border-white/5 flex flex-col justify-between shadow-2xl hover:border-brand-gold/15 transition-all duration-300">
          <div>
            ...
            <p className="text-xs text-gray-400 mb-4 font-medium leading-relaxed">Nikmati Kopi Arabika Jampang Sukabumi dan kriya premium daur ulang pemuda.</p>
            
            <div className="space-y-3">
              {featuredProducts.map((p) => (
                <div 
                  key={p.id} 
                  className="flex items-center gap-3 bg-slate-950/40 p-2.5 rounded-2xl hover:bg-slate-950/70 border border-white/5 group transition-all"
                >
                  <img 
                    src={p.imageUrl} 
                    alt={p.name} 
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-xs font-extrabold text-gray-200 truncate group-hover:text-brand-gold leading-tight">{p.name}</h4>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[9px] text-gray-500 capitalize font-mono">{p.category}</span>
        ...
        <div className="md:col-span-12 lg:col-span-5 bg-brand-card rounded-3xl p-6 border border-white/5 flex flex-col justify-between shadow-2xl min-h-[380px] hover:border-brand-gold/15 transition-all duration-300">
          <div>
            <div className="flex justify-between items-center mb-4 border-b border-white/5 pb-3">
              <div className="flex items-center gap-2">
                <Users className="w-4 h-4 text-indigo-400" />
                <h3 className="text-xs font-bold text-white font-mono uppercase tracking-wider">Direktori Talenta</h3>
              </div>
              <button 
                onClick={() => setActiveTab('talents')} 
                className="text-[10px] text-gray-400 hover:text-brand-gold uppercase font-mono tracking-widest font-black cursor-pointer"
              >
                Lihat Semua
              </button>
            </div>
            
            <p className="text-xs text-gray-400 mb-4 font-medium leading-relaxed">Jelajahi portofolio & komoditas pemuda Sukabumi. Mulai berkolaborasi.</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {talents.slice(0, 4).map((talent) => (
                <div 
                  ...
                  className="flex items-center gap-3 bg-slate-950/30 p-2.5 rounded-2xl hover:bg-slate-950/70 hover:border-brand-gold/25 transition-all border border-white/5 cursor-pointer group"
                >
                  <img 
                    src={talent.avatarUrl} 
                    alt={talent.name} 
                    referrerPolicy="no-referrer" 
                    className="w-10 h-10 rounded-full object-cover border border-white/15 group-hover:border-brand-gold/50 transition-colors" 
                  />
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-gray-200 truncate group-hover:text-brand-gold leading-tight">{talent.name}</p>
                    <p className="text-[10px] text-gray-500 font-mono truncate uppercase leading-none mt-1">{talent.field.split(' ')[0]}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-4 pt-3 border-t border-white/5">
            {talents.slice(0, 1).map((spotlight) => (
              <div 
                key={spotlight.id} 
                className="flex items-center justify-between bg-brand-gold/5 p-3 rounded-2xl border border-brand-gold/10"
              >
                <div className="flex items-center gap-2 min-w-0">
                  <div className="w-2 h-2 rounded-full bg-amber-500 animate-pulse shrink-0" />
                  <span className="text-[9px] uppercase font-mono text-gray-400 italic font-bold shrink-0">Sorotan Utama:</span>
        ...
        <div className="md:col-span-12 lg:col-span-7 bg-brand-card rounded-3xl p-6 border border-white/5 flex flex-col md:flex-row gap-5 shadow-2xl min-h-[380px] hover:border-brand-gold/15 transition-all duration-300">
          ...
          <div className="flex-1 flex flex-col justify-between border-b md:border-b-0 md:border-r border-white/5 pb-4 md:pb-0 md:pr-5">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-xs font-bold uppercase text-rose-500 tracking-wider font-mono">Acara Terdekat</h4>
              </div>
              
              {nextEvent ? (
                <div className="space-y-2 mt-4">
                  <span className="px-2 py-0.5 bg-rose-500/15 text-rose-400 text-[9px] font-bold font-mono uppercase rounded border border-rose-500/25">Tiket Tersedia</span>
                  <h3 className="text-base font-extrabold text-white leading-snug">{nextEvent.title}</h3>
                  <p className="text-xs text-gray-400 leading-relaxed line-clamp-2">{nextEvent.description}</p>
                  
                  <div className="text-[10px] text-gray-200 font-mono flex items-center gap-1.5 mt-2 bg-slate-950/40 p-2 rounded-xl border border-white/5">
            ...
            <button 
              onClick={() => setActiveTab('events')} 
              className="w-full mt-4 py-2 bg-white/5 hover:bg-white/10 text-white border border-white/10 hover:border-brand-gold/25 rounded-xl text-xs font-mono font-bold uppercase cursor-pointer transition-colors"
            >
              Jadwal Giat Seni
            </button>
          </div>
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-3 pb-2 border-b border-white/5 justify-between">
                <div className="flex items-center gap-1.5">
                  ...
                  <h4 className="text-xs font-bold uppercase text-gray-400 tracking-wider font-mono">Forum Curah Gagasan</h4>
                </div>
                <span className="text-[8px] font-mono text-emerald-400 px-1.5 py-0.5 bg-emerald-500/10 rounded font-bold animate-pulse">Live</span>
              </div>
              
              <div className="space-y-2.5 mt-3">
                {recentPosts.length > 0 ? (
                  recentPosts.map(post => (
                    <div key={post.id} className="bg-slate-950/20 p-2.5 rounded-2xl border border-white/5 space-y-1.5">
                      <span className="text-xs text-gray-300 font-semibold italic line-clamp-2 leading-snug">"{post.content}"</span>
                      <div className="flex items-center gap-1.5 justify-between border-t border-white/5 pt-1.5">
                        <span className="text-[10px] text-indigo-300 font-mono font-bold">@{post.authorName}</span>
            ...
            <button 
              ...
              className="w-full mt-4 py-2 bg-brand-gold/10 hover:bg-brand-gold hover:text-brand-charcoal text-brand-gold border border-brand-gold/20 hover:border-transparent rounded-xl text-xs font-mono font-bold uppercase cursor-pointer transition-colors"
            >
              Masuk Komunitas
            </button>
          </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        {[
          { icon: Users, val: talents.length || 5, label: 'Kreator Terdaftar', color: 'text-amber-400 bg-amber-500/10' },
          { icon: Palette, val: artworks.length || 6, label: 'Karya Terpajang', color: 'text-sky-400 bg-sky-500/10' },
          { icon: Utensils, val: (shopItems.length ? shopItems.reduce((a, b) => a + (b.stock || 0), 0) : 180), label: 'Stok Kopi & Merchandise', color: 'text-rose-400 bg-rose-500/10' },
          { icon: Award, val: events.length || 4, label: 'Agenda Seni Aktif', color: 'text-emerald-400 bg-emerald-500/10' }
        ].map((item, idx) => {
          ...
          return (
            <motion.div 
              ...
              key={idx} 
              className="p-5 rounded-3xl bg-brand-card border border-white/5 flex items-center gap-4 hover:border-brand-gold/25 hover:bg-slate-950 transition-all duration-300"
            >
              <div className={`p-3 rounded-2xl ${item.color} shrink-0`}>
                <IconComponent className="w-5 h-5" />
              </div>
              <div className="min-w-0">
                <span className="text-2xl font-serif font-black text-white block leading-none">{item.val}</span>
                <span className="text-[10px] font-bold text-gray-400 block mt-1 tracking-tight leading-none uppercase">{item.label}</span>
              </div>
            </motion.div>
          );
        })}
