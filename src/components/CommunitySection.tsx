...
export default function CommunitySection({ posts, setPosts, currentUser, userRole, openAuthModal, artworks, startChat }: CommunitySectionProps) {
  ...
  return (
    <div className="space-y-8 py-4 relative text-left" id="community-main-container">
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            className="fixed top-24 left-1/2 -translate-x-1/2 z-50 bg-brand-accent border border-brand-accent/25 text-white px-5 py-2.5 rounded-full text-xs font-mono font-bold shadow-2xl flex items-center gap-2"
          >
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 border-b border-gray-200 pb-8">
        <div className="space-y-2">
           <div className="flex items-center gap-2.5">
            <span className="text-xs font-mono text-brand-accent uppercase tracking-wider block bg-brand-accent/10 px-2.5 py-1 rounded border border-brand-accent/20 font-bold">
          ...
          <p className="text-xs sm:text-sm text-gray-650 max-w-2xl leading-relaxed">
        ...
        <button
          ...
          className={`px-5 py-3 rounded-xl text-xs font-black uppercase tracking-wide flex items-center gap-2 cursor-pointer transition-all duration-300 shadow-lg ${
            showCreatePost 
              ...
              : 'bg-brand-accent text-white hover:bg-brand-green shadow-brand-accent/15 hover:scale-[1.03]'
          }`}
      ...
      <AnimatePresence>
        {showCreatePost && (
          ...
          >
            {currentUser ? (
              <form onSubmit={handleCreatePost} className="p-6 md:p-8 rounded-3xl bg-white border border-gray-200 space-y-6 shadow-sm" id="new-post-form">
                <div className="flex justify-between items-center border-b border-gray-150 pb-4">
                  <h3 className="text-sm font-bold font-mono text-brand-accent uppercase tracking-widest flex items-center gap-2 font-black"><MessageSquare className="w-4 h-4" /> Formulir Gagasan Baru</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                   <div className="space-y-1.5">
                    <label className="text-[10px] font-mono text-gray-500 uppercase tracking-wider block">Masuk sebagai</label>
                    <div className="px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs text-brand-accent font-bold">
                      {currentUser.displayName || 'Kreator Adiksi'}
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-mono text-gray-500 uppercase tracking-wider block">Kategori Forum</label>
                    <select value={newPostGroup} onChange={(e: any) => setNewPostGroup(e.target.value)} className="w-full px-3.5 py-2.5 bg-white border border-gray-200 rounded-xl text-xs text-gray-900 focus:outline-none focus:border-brand-accent">
                ...
                <div className="space-y-4 font-sans">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-mono text-gray-500 uppercase tracking-wider block">Judul Gagasan</label>
                    <input type="text" required placeholder="Cth: Proyek Mural Kemerdekaan di Tembok Citepus" value={newPostTitle} onChange={(e) => setNewPostTitle(e.target.value)} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand-accent font-semibold" />
                  </div>
                  
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-mono text-gray-500 uppercase tracking-wider block">Deskripsi Lengkap</label>
                    <textarea rows={4} required placeholder="Jelaskan ide, tujuan, dan apa yang Anda butuhkan dari komunitas..." value={newPostContent} onChange={(e) => setNewPostContent(e.target.value)} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-xs text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand-accent resize-y leading-relaxed" />
                  </div>
                  ...
                  {newPostGroup === 'Kolaborasi' && (
                    <motion.div 
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-250 space-y-4"
                    >
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-mono text-emerald-800 uppercase tracking-wider block font-bold">📍 Lokasi Target Kolaborasi</label>
                        <input 
                          type="text" 
                          required 
                          ...
                          className="w-full px-3.5 py-2.5 bg-white border border-gray-205 rounded-xl text-xs text-gray-900 placeholder-gray-400 focus:outline-none focus:border-emerald-600"
                        />
                      </div>

                      <div className="space-y-2">
                        <label className="text-[10px] font-mono text-emerald-800 uppercase tracking-wider block font-bold">✨ Pilih Peran Kerja Kreatif Yang Dibutuhkan</label>
                        <p className="text-[10px] text-gray-600 leading-tight">Klik pada peran berikut untuk merekrut bantuan tim:</p>
                              ...
                              <button
                                ...
                                className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition ${
                                  isSelected 
                                    ...
                                    : 'bg-white border-gray-205 text-gray-500 hover:text-gray-900'
                                }`}
                              >
                                {isSelected ? '✓ ' : '+ '} {role}
                              </button>
                  ...
                  {newPostGroup === 'Workshop' && (
                    <motion.div 
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="p-4 bg-violet-50/50 rounded-2xl border border-violet-200 grid grid-cols-1 md:grid-cols-3 gap-4"
                    >
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-mono text-violet-800 uppercase tracking-wider block font-bold font-serif">📅 Waktu & Tanggal Agenda</label>
                        <input 
                          type="text" 
                          required 
                          ...
                          className="w-full px-3.5 py-2.5 bg-white border border-gray-205 rounded-xl text-xs text-gray-900 placeholder-gray-400 focus:outline-none focus:border-violet-600"
                        />
                      </div>

                      <div className="space-y-1.5">
                        <label className="text-[10px] font-mono text-violet-800 uppercase tracking-wider block font-bold font-serif">💰 Biaya Pendaftaran</label>
                        <input 
                          type="text" 
                          required 
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-mono text-violet-800 uppercase tracking-wider block font-bold font-serif">👥 Kuota Pendaftar Maksimal</label>
                        <input 
                          type="number" 
                          required 
                          placeholder="Cth: 20" 
                  ...
                  {newPostGroup === 'Karya' && (
                    <motion.div 
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="p-4 bg-emerald-50/20 rounded-2xl border border-brand-accent/25 space-y-3"
                    >
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-mono text-brand-accent uppercase tracking-wider block font-bold">🎨 Aliran & Media Karya Seni</label>
                        <input 
                          type="text" 
                          required 
                          ...
                          className="w-full px-3.5 py-2.5 bg-white border border-gray-205 rounded-xl text-xs text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand-accent"
                        />
                      </div>
                    </motion.div>
                  )}
                  ...
                  {true && (
                    <motion.div 
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="p-4 bg-gray-50 border border-gray-200 rounded-2xl space-y-4"
                    >
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-mono text-brand-accent uppercase tracking-wider block font-bold">🖼️ Unggah atau Pilih Gambar Visualisasi</label>
                        <p className="text-[10px] text-gray-650 leading-snug">Silakan pilih dari salah satu preset bertema pesisir di bawah ini, unggah karya orisinil Anda, atau masukkan tautan URL gambar eksternal.</p>
                      </div>
                          ...
                          return (
                            <div 
                              ...
                              className={`relative aspect-[3/2] rounded-lg overflow-hidden cursor-pointer border-2 transition ${
                                isSelected ? 'border-brand-accent scale-[1.03]' : 'border-transparent hover:border-gray-300'
                              }`}
                              title={img.title}
                            >
                      ...
                      <div className="border-t border-gray-205 pt-3 space-y-3">
                        <span className="text-[9px] font-mono text-gray-500 block font-bold uppercase">ALAT PENDUKUNG MEDIA:</span>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <span className="text-[10px] text-gray-500 block font-mono font-semibold">Tautkan Teks URL Gambar Mandiri:</span>
                            <input 
                              type="text" 
                          ...
                          <div className="space-y-1 bg-white border border-gray-200 p-2 rounded-xl">
                            <span className="text-[10px] text-gray-500 block font-mono mb-1.5 font-bold">Unggah Gambar Langsung ke Cloud:</span>
                            <CloudUploader 
                              onUploadSuccess={(url) => {
                                setCustomImgUrl(url);
                                setSelectedPresetImg('');
                        ...
                        {(customImgUrl || selectedPresetImg) && (
                          <div className="p-2.5 bg-brand-accent/5 border border-brand-accent/15 rounded-xl flex items-center gap-3.5">
                            <img 
                            ...
                            <div className="truncate">
                              <span className="text-[9px] font-mono text-brand-accent font-bold block uppercase">Visual Tersemat Aktif</span>
                              <span className="text-[11px] text-gray-600 font-medium block truncate max-w-sm">{customImgUrl || "Dari preset galeri pesisir"}</span>
                            </div>
                          </div>
                        )}
                      </div>
                ...
                <div className="flex justify-end items-center gap-3 border-t border-gray-205 pt-4">
                  <button type="button" onClick={() => setShowCreatePost(false)} className="px-4 py-2 hover:bg-gray-100 rounded-xl text-xs text-gray-500 hover:text-gray-900 transition-colors">Batal</button>
                  <button type="submit" disabled={isSubmitting} className="px-6 py-2.5 bg-brand-accent hover:bg-brand-green disabled:bg-gray-200 text-white font-black rounded-xl text-xs flex items-center gap-2 transition-all shadow-md">{isSubmitting ? "Mengirim..." : "Publikasikan"} <Send className="w-3.5 h-3.5" /></button>
                </div>
              </form>
            ) : (
              <div className="p-8 rounded-3xl bg-white border border-rose-250 text-center space-y-4 shadow-sm">
                <Lock className="w-8 h-8 mx-auto text-rose-500" />
                <h3 className="font-serif font-bold text-xl text-gray-900">Akses Forum Terbatas</h3>
                <p className="text-xs text-gray-650 max-w-sm mx-auto">Untuk menjaga kualitas dan keamanan diskusi, Anda harus masuk terlebih dahulu untuk dapat membuat postingan baru.</p>
                <button onClick={openAuthModal} className="mt-2 px-5 py-2.5 bg-brand-accent text-white font-mono font-bold text-xs rounded-xl hover:bg-brand-green transition-colors shadow-lg">Masuk atau Daftar Sekarang</button>
              </div>
            )}
          </motion.div>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-[160px]">
          ...
          <div className="bg-white border border-gray-200 p-5 rounded-3xl space-y-4 shadow-sm">
            <span className="text-[10px] font-mono font-extrabold text-brand-accent uppercase tracking-wider block">CARI GAGASAN</span>
            ...
            <div className="relative">
              <input 
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand-accent font-medium"
              />
              <Search className="w-4 h-4 text-gray-450 absolute left-3 top-3" />
              {searchQuery && (
                <button 
                  onClick={() => setSearchQuery('')}
                  className="absolute right-3 top-3 text-gray-450 hover:text-gray-900"
                >
            ...
            <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-200">
              <button 
                ...
                className={`flex-1 py-1.5 rounded-lg text-[10px] font-mono font-bold uppercase transition ${sortBy === 'terbaru' ? 'bg-brand-accent text-white shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
              >
                Terbaru
              </button>
              <button 
                onClick={() => setSortBy('terpopuler')}
                className={`flex-1 py-1.5 rounded-lg text-[10px] font-mono font-bold uppercase transition ${sortBy === 'terpopuler' ? 'bg-brand-accent text-white shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
              >
                Terpopuler
              </button>
            </div>
          ...
          <div className="bg-white border border-gray-200 p-5 rounded-3xl space-y-3 shadow-sm">
            <span className="text-[10px] font-mono font-extrabold text-brand-accent uppercase tracking-wider block">KATEGORI FILTER</span>
            
            <div className="space-y-1.5 font-sans">
              {[
                { key: 'Semua', label: '🌍 Semua Diskusi', count: posts.length, color: 'text-zinc-500' },
                { key: 'Diskusi', label: '💬 Tanya & Diskusi', count: posts.filter(p => p.group === 'Diskusi').length, color: 'text-sky-500' },
                { key: 'Kolaborasi', label: '🎨 Kolaborasi Aktif', count: posts.filter(p => p.group === 'Kolaborasi').length, color: 'text-emerald-500' },
                { key: 'Workshop', label: '🎓 Kelas Workshop', count: posts.filter(p => p.group === 'Workshop').length, color: 'text-violet-500' },
                { key: 'Karya', label: '🖼️ Pameran Karya', count: posts.filter(p => p.group === 'Karya').length, color: 'text-brand-accent' }
              ].map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex justify-between items-center px-3.5 py-2.5 rounded-xl text-left text-xs font-semibold cursor-pointer transition ${isActive ? 'bg-brand-accent/15 border border-brand-accent/30 text-brand-accent' : 'bg-gray-50/50 border border-gray-205 hover:bg-gray-100 text-gray-600 hover:text-gray-900'}`}
                  >
                    <span className="truncate">{cat.label}</span>
                    <span className={`text-[10px] font-mono px-2 py-0.5 rounded-md font-bold ${isActive ? 'text-brand-accent bg-brand-accent/10' : 'text-gray-500 bg-gray-100'}`}>
                      {cat.count}
                    </span>
                  </button>
                );
          ...
          <div className="bg-white border border-gray-200 p-5 rounded-3xl space-y-4 shadow-sm">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] font-mono font-extrabold text-gray-500 uppercase tracking-wider">EKOSISTEM KREATIF</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3.5 pt-1">
              <div className="p-3 bg-gray-50 rounded-2xl border border-gray-200">
                ...
                <span className="text-xl font-serif font-black text-brand-accent block">{totalGagasan}</span>
              </div>
              <div className="p-3 bg-gray-50 rounded-2xl border border-gray-200">
                <span className="text-[10px] font-mono text-gray-500 block font-bold text-gray-400">KOLABORASI</span>
                <span className="text-xl font-serif font-black text-gray-900 block">{activeCollaborations}</span>
              </div>
              <div className="p-3 bg-gray-50 rounded-2xl border border-gray-200">
                <span className="text-[10px] font-mono text-gray-500 block font-bold text-gray-400">WORKSHOP</span>
                <span className="text-xl font-serif font-black text-gray-900 block">{upcomingWorkshops}</span>
              </div>
              <div className="p-3 bg-gray-50 rounded-2xl border border-gray-200">
                <span className="text-[10px] font-mono text-gray-500 block font-bold text-gray-400">APRESIASI</span>
                <span className="text-xl font-serif font-black text-rose-550 block">♥ {totalLikes}</span>
              </div>
            </div>
          </div>

          {/* 4. Creative Leaderboard (Community Stars) */}
          <div className="bg-white border border-gray-200 p-5 rounded-3xl space-y-3.5 shadow-sm">
            <span className="text-[10px] font-mono font-extrabold text-brand-accent uppercase tracking-wider block">KREATOR TERAKTIF</span>
            
            <div className="space-y-3">
              {communityStars.length > 0 ? (
                communityStars.map((star, idx) => (
                  <div key={idx} className="flex items-center justify-between p-2 bg-gray-50/70 hover:bg-gray-100 border border-gray-150 hover:border-brand-accent/20 rounded-2xl transition">
                    <div className="flex items-center gap-2.5 truncate">
                      <div className="relative shrink-0">
                        <img 
                          ...
                          referrerPolicy="no-referrer"
                        />
                        <span className="absolute -top-1 -left-1 w-4 h-4 bg-brand-accent text-white font-sans text-[9px] font-black rounded-full flex items-center justify-center border border-white">
                          {idx + 1}
                        </span>
                      </div>
                      <div className="truncate space-y-0.5">
                        <span className="text-xs font-bold text-gray-900 block truncate leading-tight">{star.name}</span>
                        <span className="text-[9px] text-gray-500 font-mono block truncate uppercase">{star.field}</span>
                      </div>
                    </div>
                    <div className="text-right pl-2 shrink-0">
                      <span className="text-[10px] font-semibold text-brand-accent font-mono block leading-none font-bold">★ {star.score}</span>
                      <span className="text-[8px] text-gray-500 font-mono block leading-none mt-1">{star.count} posts</span>
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-[10px] font-mono text-gray-500 text-center py-2">Belum ada peringkat kreator.</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 p-5 rounded-3xl space-y-4 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-mono font-extrabold text-brand-accent uppercase tracking-wider block">VIRTUAL ROOM MANAGER</span>
              <span className={`w-2 h-2 rounded-full ${activeMeetRooms.length > 0 ? "bg-emerald-500 animate-pulse" : "bg-gray-400"}`} title="Sistem Aktif" />
            </div>
            ...
            <div className="flex items-center justify-between text-[10px] bg-gray-50 border border-gray-200 py-2 px-3 rounded-2xl font-mono text-gray-600 shadow-sm">
              <div className="flex items-center gap-1.5 truncate">
                ...
                <span className="truncate font-bold">Sync: {isSyncingState ? "Verifikasi..." : "Aktif"}</span>
              </div>
              <div className="flex items-center gap-2 font-mono text-[9px] font-bold">
                <span className="text-gray-450">Ref: {lastSyncedTime.toLocaleTimeString()}</span>
                <button
                  type="button"
                  onClick={verifyMeetRoomsAsync}
                  disabled={isSyncingState}
                  className="text-brand-accent hover:text-brand-green transition disabled:opacity-40 uppercase font-black pb-0.5 cursor-pointer"
            ...
            {workspacePermissionError && (
              <div className="p-3.5 bg-rose-50 border border-rose-200 text-rose-850 rounded-2xl space-y-2.5 text-[11px] leading-relaxed">
                ...
                <p className="font-sans leading-relaxed text-gray-700">{workspacePermissionError}</p>
                <div className="pt-1 flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl text-[9px] font-bold uppercase transition"
                  >
                    Abaikan
                  </button>
                </div>
              ...
              {activeMeetRooms.length > 0 ? (
                <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
                  {activeMeetRooms.map((room) => {
                    ...
                    return (
                      <div key={room.id} className="p-3 bg-gray-50 border border-gray-200 hover:border-brand-accent/20 rounded-2xl space-y-2.5 transition duration-300 shadow-sm">
                        <div className="flex justify-between items-start gap-2">
                          <div className="space-y-1 truncate">
                            <span className="flex items-center gap-1 text-[8px] font-mono font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full w-max uppercase tracking-wider">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping mr-0.5" />
                              Live Open
                            </span>
                            <h4 className="text-xs font-bold text-gray-900 font-serif tracking-tight leading-snug truncate" title={room.title}>
                              {room.title}
                            </h4>
                          </div>
                          ...
                          {isCreator && (
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-rose-500/15 hover:bg-rose-500 text-rose-500 hover:text-white transition cursor-pointer shrink-0"
                        ...
                        <div className="flex items-center justify-between pt-1 border-t border-gray-150 gap-2">
                          {/* Host info */}
                          <div className="flex items-center gap-1.5 truncate max-w-[60%]">
                            <img
                              src={room.hostAvatar || selectedAvatar}
                              alt={room.hostName}
                              className="w-5.5 h-5.5 rounded-full object-cover border border-gray-205"
                              referrerPolicy="no-referrer"
                            />
                            <span className="text-[10px] text-gray-550 truncate leading-none font-bold">{room.hostName}</span>
                          </div>
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-1 px-3 py-1.5 bg-brand-accent hover:bg-brand-green text-white text-[10px] font-black uppercase rounded-lg transition shadow cursor-pointer active:scale-95"
                          >
